...
export default class Gestionnaire {
  ...
  public datePartie: Date = new Date(); // Date par défaut, sera remplacée par la date de la sauvegarde si elle existe
  public numeroPartie: number = 0;
  private _motATrouver: string;
  private _compositionMotATrouver: { [lettre: string]: number };
  public maxNbPropositions: number = 6;
  public victoire: boolean = false;
  public perdu: boolean = false;
  public longueurMot: number = 0;
  public premiereLettre: string = "";
  public motActuel = 0;
  ...
  private chargerPartieEnCours(): void {
    let dataPartieEnCours = localStorage.getItem("partieEnCours");
    if (!dataPartieEnCours) return; // Utiliser les valeurs par défaut

    let partieEnCours = JSON.parse(dataPartieEnCours) as PartieEnCours;
    let aujourdhui = new Date();
    let dateSauvegarde = new Date(partieEnCours.datePartie);
    if (
      aujourdhui.getDate() !== dateSauvegarde.getDate() ||
  ...
  private calculerNumeroPartie(date: Date): number {
  ...
  private choisirMot(numeroPartie: number): string {
  ...
  public actualiserAffichageMotSaisi(mot: string): void {
